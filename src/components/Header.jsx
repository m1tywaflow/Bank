...
export default function Header() {
  ...
  const [nickname, setNickname] = useState(null);
    ...
    // hide header + close menus on scroll
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > lastScrollY) {
        setShow(false);
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between w-[90%] max-w-6xl px-6 py-3 mt-4 bg-[#111] rounded-full border border-gray-800 shadow-lg">
        ...
        {/* Desktop Nav */}
        <nav className="hidden md:flex gap-6 text-lg">{navLinks}</nav>
      ...
      {/* Mobile Menu */}
      {mobileMenu && (
        <div className="absolute top-full left-0 w-full bg-[#111] border-t border-gray-800 shadow-lg md:hidden flex flex-col items-center gap-4 py-6 text-lg">
          {navLinks}
          {user ? (
            <>
              <NavLink
            ...
            <>
              ...
              <button className="px-6 py-2 rounded-full bg-lime-400 text-black font-medium text-sm hover:bg-lime-300 transition">
                Login
              </button>
            </>
          )}
